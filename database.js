...
const allowedTables = [
  ...
  "shares",
  "followers",
]; // Tambahkan semua nama tabel Anda di sini
...
app.post("/:table", validateTable, (req, res) => {
  ...
  pool.query(`INSERT INTO ?? SET ?`, [table, data], (err, result) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ error: `Gagal memasukkan ke ${table}`, details: err.message });
    }
...
app.get("/", (req, res) => {
  const apiGuide = `
        ...
        <ul>
            <li>GET <code>/${
              allowedTables[0]
            }</code> - Mengambil semua data dari tabel "${
    allowedTables[0]
  }".</li>
            <li>GET <code>/${
              allowedTables[0]
            }/:id</code> - Mengambil data berdasarkan ID dari tabel "${
    allowedTables[0]
  }".</li>
            <li>POST <code>/${
              allowedTables[0]
            }</code> - Menambahkan data baru ke tabel "${
    allowedTables[0]
  }".</li>
            <li>PUT <code>/${
              allowedTables[0]
            }/:id</code> - Memperbarui data berdasarkan ID di tabel "${
    allowedTables[0]
  }".</li>
            <li>DELETE <code>/${
              allowedTables[0]
            }/:id</code> - Menghapus data berdasarkan ID dari tabel "${
    allowedTables[0]
  }".</li>
        </ul>
        <P>Anda dapat menambahkan parameter query ke endpoint GET untuk memfilter data. Contoh: <code>/${
          allowedTables[0]
        }?name=arline&&email=zero@mail.com</code></p>
    `;
  res.send(apiGuide);
});
